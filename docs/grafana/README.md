...
| `llamacpp-dashboard.json` | llama.cpp runtime observability: recent (5m rate) vs lifetime-avg prompt/generation throughput, request queueing and busy decode slots, decode ops, prompt/decode duty cycle, and peak observed context length. Grouped by service, namespace; templated on a `$service` variable. No KV-cache panel since llama.cpp exposes no KV-cache utilization series. |
