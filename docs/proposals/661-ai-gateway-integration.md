...
### 5.4 Apply-time validation (ModelRouter validating webhook)

Every honest-boundary check the compiler accumulated (unsupported match, unsupported budget field/scope, invalid auth, invalid authorization, unsupported auditLog, unsafe sensitive route) is surfaced today only on the ModelRouter status after a reconcile (`GatewayReady=False`). A misconfigured router applies cleanly and then silently does nothing. This slice promotes those checks to a **validating admission webhook** so the mistake is rejected at `kubectl apply` time with the same message.

- **Reuse, do not duplicate.** The check functions (`unsupportedMatchMessage`, `unsupportedBudgetMessage`, `invalidAuthMessage`, `invalidAuthorizationMessage`, `unsupportedAuditLogMessage`, `unsafeSensitiveRouteMessage`) are already pure (they read `mr.Spec` only, no client calls). The validator lives in `internal/controller` alongside them and calls them directly, so the webhook and the reconciler can never diverge. It also runs the existing spec validation (`validateModelRouter`). The gateway-mode checks apply only when `spec.dataPlane: Gateway`.
- **Boundary: pure checks only.** Checks that need a cluster lookup (a backend referencing a missing InferenceService; a backend that resolves to a non-local tier) live in `resolveBackends` and stay reconciler-only: a webhook cannot reliably do cross-object reads under all apiserver conditions, and CRD validation already rejects `tier: cloud` on in-cluster backends. The webhook catches config-level mistakes; the reconciler catches live-cluster ones. The reconciler keeps all its status checks (defense in depth, and the webhook can be disabled).
- **Infrastructure (mirrors `charts/foreman`).** The operator binary already runs a controller-runtime webhook server (`--webhook-cert-path`). This slice adds the helm side `charts/llmkube` has never had: a self-signed serving-cert Secret generated with lookup-reuse (stable caBundle across upgrades), a webhook Service, the `ValidatingWebhookConfiguration` (group `inference.llmkube.dev`, resource `modelrouters`, ops CREATE+UPDATE), the operator-deployment cert mount + port, and a `webhook` values stanza (`enabled`, `failurePolicy` default `Fail`, `certValidityDays`, `port`), all gated on `webhook.enabled` so a webhook-less install is unaffected. Registration in `cmd/main.go` is gated on the cert being present, degrading to no-webhook rather than a crash loop (the foreman pattern).
- **Deferred:** an InferenceService gateway-exposure validator (the MVP path has its own fail-loud) is a later addition on the same webhook infrastructure; this slice scopes to ModelRouter.
